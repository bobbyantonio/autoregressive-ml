...
if __name__ == '__main__':
    ...
    if data.HI_RES_ERA5_DIR == args.output_dir:
        raise ValueError('Output folder mush be different from input')
    
    if args.vars is not None:
        var_collections = {'surface': [item for item in args.vars if item in data.ERA5_SURFACE_VARS + data.ERA5_STATIC_VARS], 
                        'plevels': [item for item in args.vars if item in data.ERA5_PLEVEL_VARS]}
    else:
        var_collections = {'surface': data.ERA5_SURFACE_VARS + data.ERA5_STATIC_VARS, 
                        'plevels': data.ERA5_PLEVEL_VARS}
                ...
                for month in args.months:
                    ...
                    for day in days:
                        ...
                        for method in ['bilinear', 'conservative']:
                            ...
                            fp_in = os.path.join(data.HI_RES_ERA5_DIR, k, era5_var, year,  suffix)
                            
                            folder_out = os.path.join(args.output_dir, method, k, era5_var, year)
